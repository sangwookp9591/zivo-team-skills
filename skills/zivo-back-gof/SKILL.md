...
// Strategy Registry (Spring이 모든 구현체를 자동 수집)
@Component
...
public abstract class AbstractExportService<T> {
    public final byte[] export(ExportRequest request) {
        ...
        List<String[]> rows = transform(data);
        return render(rows, request.getFormat());
    }
    protected abstract List<T> fetchData(ExportRequest request);
    protected abstract List<String[]> transform(List<T> data);
    private byte[] render(List<String[]> rows, ExportFormat format) { /* 공통 */ }
}
```

### 4. Observer — Spring Event 기반

```java
public record OrderCompletedEvent(Long orderId, Long userId, BigDecimal amount) {}

// 발행
eventPublisher.publishEvent(new OrderCompletedEvent(order.getId(), order.getUserId(), order.getAmount()));

// 리스너 (독립적 후속 처리)
@TransactionalEventListener(phase = TransactionPhase.AFTER_COMMIT)
@Async
public void handle(OrderCompletedEvent event) { /* 포인트 적립 */ }
```
